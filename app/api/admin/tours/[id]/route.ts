...
function sanitizeTourData(body: any) {
  const data: any = {};
  if (body.name !== undefined) data.name = body.name || "Untitled Tour";
...
export async function GET(_: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const item = await prisma.tourService.findUnique({ where: { id } });
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const cleanData = sanitizeTourData(body);
    const item = await prisma.tourService.update({
      where: { id },
    ...
    return NextResponse.json({ success: false, error: error?.message || "Failed to update tour" }, { status: 500 });
  }
}
...
export async function DELETE(_: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    return NextResponse.json({ success: false, error: error?.message || "Failed to delete tour" }, { status: 500 });
  }
}
