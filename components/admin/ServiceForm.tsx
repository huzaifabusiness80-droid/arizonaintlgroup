...
const S: Record<string, React.CSSProperties> = {
  form: { fontFamily: "system-ui, -apple-system, sans-serif", maxWidth: 840 },
...
export default function ServiceForm({
  ...
  // Process Steps
  function addStep() {
    update("processSteps", [...(form.processSteps || []), { ...EMPTY_STEP }]);
  }
  ...
  function removeStep(i: number) {
    update(
      "processSteps",
      (form.processSteps || []).filter((_, idx) => idx !== i)
    );
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (data.success) {
        setStatus({
          type: "success",
          msg: mode === "edit" ? "Saved successfully." : "Created successfully.",
        });
  ...
  return (
    <form onSubmit={handleSubmit} style={S.form}>
      {status && (
        <div
          style={{
            ...
            border: `1px solid ${status.type === "success" ? "#c8e6c9" : "#ffcccc"}`,
            color: status.type === "success" ? "#2e7d32" : "#c00",
          }}
        >
          {status.msg}
        </div>
      ...
      {type !== "visa" && (
        <ImageUploadInput
          label="Main Image (Upload from Computer)"
          value={form.image}
          onChange={(url) => update("image", url)}
          folder={`arizonaintl/${type}`}
        />
      )}
      ...
      {type === "visa" && (
        <>
          ...
          <div style={S.row}>
            <ImageUploadInput
              label="Hero Image (Upload from Computer)"
              value={form.heroImage || ""}
              onChange={(url) => update("heroImage", url)}
              folder="arizonaintl/visas"
            />
            <ImageUploadInput
              label="Card Image (Upload from Computer)"
          ...
          <button type="button" style={S.addBtn} onClick={addRequirement}>
            + Add Requirement
          </button>
          ...
          {(form.included || []).map((inc, i) => (
            <div key={i} style={{ display: "flex", gap: 8, marginBottom: 8 }}>
              <input
                ...
                placeholder="e.g. BLS Slot Booking"
              />
              <button
                type="button"
                style={S.removeBtn}
          ...
          <button type="button" style={S.addBtn} onClick={addIncluded}>
            + Add Included Item
          </button>
        </>
      )}
      ...
      <button type="button" style={S.addBtn} onClick={addOption}>
        + Add Pricing Option
      </button>

      {/* Gallery with Direct Computer Upload */}
      <div style={S.sectionTitle}>Gallery Photos (Upload from Computer)</div>
      <p style={{ fontSize: 12, color: "#999", marginBottom: 12 }}>
        Click "+ Upload Photos" to select one or multiple images from your computer.
      </p>
      <GalleryUpload
        images={form.gallery}
        onChange={(images) => update("gallery", images)}
        folder={`arizonaintl/${type}/gallery`}
      />
      ...
      <div style={{ marginTop: 32, paddingTop: 24, borderTop: "1px solid #eee" }}>
        <button
          type="submit"
          disabled={saving}
          style={{ ...S.submitBtn, background: saving ? "#888" : "#111" }}
        >
